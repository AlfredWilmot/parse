...
var (
	...
	ErrBencodingInt64      = errors.New("invalid bencodingByteString")
...
// a collection of bytes with a label indicating the type of bencoding data
...
func newBencodingByteString(data []byte) ([]byte, error) {
	...
	case data[0] == '0' && data[1] == ':':
		return nil, nil // empty ByteString is allowed
...
readByteStringLengthSegment:
	for {
		...
		default:
			return data[:cursor], fmt.Errorf("only digits are allowed in the length portion of a ByteString (%v)", string(data[:cursor+1]))
		}
		if data[cursor] != ':' && cursor == uint64(len(data)-1) {
			return data[:cursor], fmt.Errorf("exhaused input buffer but ByteString delimeter ':' never reached (%s)", string(data))
		}
	}
	...
	if err != nil {
		return data[:cursor], fmt.Errorf("invalid uint64 (%s): %v", lenData, err)
	}
...
func newBencodingInt64(data []byte) ([]byte, error) {
	...
	for cursor < len(data) {
		...
		case b == 'e':
			...
			if err != nil {
				return data[:cursor], fmt.Errorf("could not parse %s into int64", string(data))
			}
